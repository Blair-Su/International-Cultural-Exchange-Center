    ...
    <div className="relative w-full h-[300px] sm:h-[380px] lg:h-[480px] overflow-hidden">
      ...
      <div className="relative flex flex-col justify-end h-full max-w-[1248px] mx-auto px-4 sm:px-6 lg:px-12 pb-10 sm:pb-14 lg:pb-16">
        ...
        <h1 className="font-['Inter:Semi_Bold',sans-serif] font-semibold text-[28px] sm:text-[44px] lg:text-[64px] text-white tracking-[-1.28px] leading-[1.2] max-w-[700px]">
          {isSimplified ? chineseCopy("文化课程") : "Cultural Classes"}
        </h1>
      </div>
    </div>
  );
  ...
  return (
    ...
    >
      <span className="font-['Inter:Medium',sans-serif] font-medium text-white text-[15px] lg:text-[18px] whitespace-nowrap">
        {isSimplified ? chineseCopy("咨询详情") : "Contact for Details"} →
      </span>
    </button>
  );
}
          ...
          <div className="flex flex-col gap-8 w-full lg:flex-1">
            <div>
              <h5 className="font-['Inter:Semi_Bold',sans-serif] font-semibold text-[20px] text-[#E48D62] tracking-[-0.48px] leading-[1.45]">
                {isSimplified ? chineseCopy("参与课程") : "Cultural Classes"}
              </h5>
              <h2 className="font-['Inter:Semi_Bold',sans-serif] font-semibold text-[28px] sm:text-[36px] lg:text-[44px] text-black tracking-[-0.96px] leading-[1.25] mt-1">
                {isSimplified ? chineseCopy("从一次体验开始，走近文化。") : "Immerse Yourself in Culture"}
              </h2>
            </div>
            <p className="font-['Inter:Regular',sans-serif] font-normal text-[15px] lg:text-[18px] text-[rgba(0,0,0,0.68)] leading-[1.6]">
              {isSimplified ? (
                <>
                  ...
                  {chineseCopy("可以听见。")}<br />
                  {chineseCopy("可以看见。")}<br />
                  {chineseCopy("也可以亲身体验。")}<br />
                  {chineseCopy("从传统舞蹈、古风歌曲，到汉服文化与礼仪课程，")}<br />
                  {chineseCopy("邀请不同年龄、不同文化背景的人们，")}<br />
                  {chineseCopy("一起学习、分享，理解。")}
                </>
              ) : (
  ...
  return (
    <section className="bg-[#f8f7f5] w-full">
      <div className="max-w-[1248px] mx-auto px-4 sm:px-6 lg:px-12">
        ...
        <ClassCard
          eyebrow={isSimplified ? chineseCopy("从舞蹈认识文化") : "Dance"}
          title={isSimplified ? chineseCopy("民族舞课程") : "Dance Class"}
          img={danceImg}
          imgAlt="Dance class"
          ...
          body={
            isSimplified ? (
              <p>
                舞蹈，<br />
                {chineseCopy("也是一种文化语言。")}<br />
                {chineseCopy("从传统到当代，")}<br />
                {chineseCopy("在身体的律动中感受文化，")}<br />
                {chineseCopy("在学习与交流中发现更多可能。")}<br />
                {chineseCopy("无论是否拥有舞蹈基础，")}<br />
                {chineseCopy("都欢迎从这里开始。")}
              </p>
            ) : (
              <p>
        ...
        <ClassCard
          eyebrow={isSimplified ? chineseCopy("从音乐认识文化") : "Music"}
          title={isSimplified ? chineseCopy("古风声乐课程") : "Traditional Chinese Songs Class"}
          img={songsImg}
          ...
          body={
            isSimplified ? (
              <p>
                {chineseCopy("歌声，")}<br />
                {chineseCopy("承载着文化，")}<br />
                {chineseCopy("也记录着时代。")}<br />
                {chineseCopy("通过学习经典歌曲，")}<br />
                {chineseCopy("走近作品背后的故事、情感与文化意蕴。")}<br />
                {chineseCopy("让旋律成为交流的语言，")}<br />
                {chineseCopy("也让文化在歌声中继续传唱。")}
              </p>
            ) : (
              <p>
        ...
        <ClassCard
          eyebrow={isSimplified ? chineseCopy("从服饰认识文化") : "Hanfu"}
          title={isSimplified ? chineseCopy("汉服课程") : "Hanfu Class"}
          img={hanfuImg}
          imgAlt="Hanfu class"
          onNavigate={onNavigate}
          isSimplified={isSimplified}
          body={
            isSimplified ? (
              <p>
                {chineseCopy("一袭衣裳，")}<br />
                {chineseCopy("承载着千年的文化。")}<br />
                {chineseCopy("从历史、形制到礼仪，")}<br />
                {chineseCopy("了解汉服背后的文化内涵，")}<br />
                {chineseCopy("学习正确的穿着方式，")}<br />
                {chineseCopy("感受传统服饰之美。")}
              </p>
            ) : (
              <>
                <p className="mb-4">
